      ...
      <div className="max-w-[1280px] mx-auto px-6 lg:px-8 w-full relative z-10">
        ...
        <Reveal direction="up" delay={0.05}>
          <div className="mb-20 max-w-3xl">
            <SectionBadge en="Our Solutions" ar="حلولنا" />
            ...
            return (
              <div key={solution.id} className={`flex flex-col lg:flex-row gap-16 items-center ${isEven ? '' : 'lg:flex-row-reverse'}`}>
                ...
                <Reveal direction={isEven ? "right" : "left"} delay={0.1} className="w-full lg:w-1/2">
                  <div className="aspect-[4/3] rounded-md border border-brand-secondary bg-brand-primary/50 relative overflow-hidden shadow-2xl flex items-center justify-center group">
                    <div className="absolute inset-0 bg-gradient-to-tr from-brand-secondary/20 to-transparent"></div>
                    {Icon && (
                      <Icon className="w-32 h-32 text-brand-secondary/50 group-hover:scale-110 transition-transform duration-700" strokeWidth={1} />
                    )}
                  </div>
                </Reveal>
                  ...
                  <div className="flex flex-col">
                    {Icon && (
                      <div className="bg-brand-accent/10 w-16 h-16 rounded-full flex items-center justify-center mb-6">
                        <Icon className="text-brand-accent w-8 h-8" strokeWidth={1.5} />
                      </div>
                    )}
